...
class Searcher(AbstractSearcher):
    ...
    def __init__(self, parameters_config, n_iter, n_suggestion):
        ...
        #     self.interval += parameters_config[key]["double_max_value"] - parameters_config[key]["double_min_value"]
    ...
    def suggest(self, suggestions_history, n_suggestions=1):
        ...
        if (suggestions_history is None) or (len(suggestions_history) <= 0):
            ...
            for index in range(max(n_suggestions * 3, 10)):
                utility_function = UtilityFunction(kind='gp_poi', kappa=(index + 1) * 2.567, x_i=index*self.improve_step+0.1)
