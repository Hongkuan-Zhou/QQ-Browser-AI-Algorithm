...
class Searcher(AbstractSearcher):
    ...
    def __init__(self, parameters_config, n_iter, n_suggestion):
        ...
        gp = GaussianProcessRegressor(
            # kernel=RationalQuadratic(length_scale=1, alpha=0.5),
            kernel=Matern(length_scale=1.0, nu=0.6),
            alpha=1e-5,
            normalize_y=True,
        ...
        #     self.interval += parameters_config[key]["double_step"]
        # self.interval /= len(parameters_config)
        self.improve_step = 10
        self.decay = 0.95
    ...
    def suggest(self, suggestions_history, n_suggestions=1):
        ...
        if (suggestions_history is None) or (len(suggestions_history) <= 0):
            ...
            self.improve_step *= self.decay
            _bounds = self.get_bounds()
            suggestions = []
            suggestions_candidate = []
            print("ymax=", y_datas.max())
            print("improve_step=", self.improve_step)
            for index in range(n_suggestions*10):
                p = np.random.rand()
                if p < 0.9:
            ...
            while i < n_suggestions and j < len(suggestions_candidate):
                if not suggestions:
                    ...
                    t = False
                    for suggestion in suggestions:
                        if self.get_valid_suggestion(suggestion) == self.get_valid_suggestion(suggestions_candidate[j][0]):
                            t = True
                            break
                    if t:
                        j += 1
